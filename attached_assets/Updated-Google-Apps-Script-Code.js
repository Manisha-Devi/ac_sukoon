...
function addAddaPayment(data) {
  try {
    let sheet = SpreadsheetApp.openById(SPREADSHEET_ID).getSheetByName(SHEET_NAMES.ADDA_PAYMENTS);

    // Create sheet if it doesn't exist
    if (!sheet) {
      sheet = SpreadsheetApp.openById(SPREADSHEET_ID).insertSheet(SHEET_NAMES.ADDA_PAYMENTS);
      // Add headers exactly as specified
      sheet.getRange(1, 1, 1, 10).setValues([[
        "Timestamp", "Date", "AddaName", "CashAmount", "BankAmount", "TotalAmount", "Remarks", "SubmittedBy", "EntryType", "EntryId"
      ]]);
    }
...
function addFuelPayment(data) {
  try {
    let sheet = SpreadsheetApp.openById(SPREADSHEET_ID).getSheetByName(SHEET_NAMES.FUEL_PAYMENTS);

    // Create sheet if it doesn't exist
    if (!sheet) {
      sheet = SpreadsheetApp.openById(SPREADSHEET_ID).insertSheet(SHEET_NAMES.FUEL_PAYMENTS);
      // Add headers exactly as specified
...
function addUnionPayment(data) {
  try {
    let sheet = SpreadsheetApp.openById(SPREADSHEET_ID).getSheetByName(SHEET_NAMES.UNION_PAYMENTS);

    // Create sheet if it doesn't exist
    if (!sheet) {
      sheet = SpreadsheetApp.openById(SPREADSHEET_ID).insertSheet(SHEET_NAMES.UNION_PAYMENTS);
      // Add headers exactly as specified
      sheet.getRange(1, 1, 1, 10).setValues([[
