...
function MiscPayment({ expenseData, setExpenseData, setTotalExpenses, setCashBookEntries }) {
  ...
  useEffect(() => {
    const loadData = async () => {
      setIsLoading(true);
      try {
        console.log('🚀 Loading misc payment data from Google Sheets...');

        const [serviceResult, otherResult] = await Promise.all([
        ...
        if (serviceResult.success && Array.isArray(serviceResult.data)) {
          const serviceData = serviceResult.data.map(entry => ({
            id: entry.entryId,
          ...
          allMiscData = [...allMiscData, ...serviceData];

          // Generate cash book entries for service payments
          const serviceCashEntries = serviceData.map(entry => ({
            id: `service-${entry.entryId}`,
            date: entry.date,
            particulars: "Service Payment",
            description: `Service expense - ${entry.serviceDetails}${entry.mechanic ? ` at ${entry.mechanic}` : ''}`,
          ...
          allCashBookEntries = [...allCashBookEntries, ...serviceCashEntries];
        }

        // Process other payments
        if (otherResult.success && Array.isArray(otherResult.data)) {
          const otherData = otherResult.data.map(entry => ({
            id: entry.entryId,
            entryId: entry.entryId,
            timestamp: convertToTimeString(entry.timestamp),
            date: convertToDateString(entry.date),
          ...
          allMiscData = [...allMiscData, ...otherData];

          // Generate cash book entries for other payments
          const otherCashEntries = otherData.map(entry => ({
          ...
          allCashBookEntries = [...allCashBookEntries, ...otherCashEntries];
        }

        console.log('✅ Misc payment data loaded:', allMiscData.length, 'entries');

        // Update expense data with misc entries
        setExpenseData(prev => {
          const nonMiscData = prev.filter(entry => 
            !['service', 'other'].includes(entry.type)
          );
          return [...nonMiscData, ...allMiscData];
        });

        // Update total expenses
        const totalMiscExpenses = allMiscData.reduce((sum, entry) => sum + (entry.totalAmount || 0), 0);
        setTotalExpenses(prev => {
          const currentMiscExpenses = expenseData.filter(entry => 
            ['service', 'other'].includes(entry.type)
          ).reduce((sum, entry) => sum + (entry.totalAmount || 0), 0);
          return prev - currentMiscExpenses + totalMiscExpenses;
        });

        // Update cash book entries
        setCashBookEntries(prev => {
          const nonMiscEntries = prev.filter(entry => 
            !['service-payment', 'other-payment'].includes(entry.source)
          );
          return [...allCashBookEntries, ...nonMiscEntries];
        });

      } catch (error) {
        console.error('Error loading misc payment data:', error);
      } finally {
        setIsLoading(false);
      }
    };
