...
function FareEntry({ fareData, setFareData, setTotalEarnings, setCashBookEntries }) {
  ...
  // Load existing fare receipts on component mount
  useEffect(() => {
    loadFareReceipts();
  }, []);

  const loadFareReceipts = async () => {
    try {
      ...
      if (result.success) {
        ...
        const total = (result.data || []).reduce((sum, entry) => sum + (parseFloat(entry.totalAmount) || 0), 0);
        setTotalEarnings(total);
      } else {
        console.error('Failed to load fare receipts:', result.error);
      }
    } catch (error) {
      console.error('Error loading fare receipts:', error);
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.route || !formData.totalAmount) {
    ...
    try {
      ...
      const result = await addFareReceipt(dataToSubmit);

      if (result.success) {
        setSubmitStatus({ type: 'success', message: 'Fare receipt added successfully!' });
        ...
        setCashBookEntries(prev => [...prev, {
          ...
          date: formData.date,
          type: 'Income',
          description: `Fare Receipt - ${formData.route}`,
        ...
        setSubmitStatus({ type: 'error', message: result.error || 'Failed to add fare receipt' });
      }
    } catch (error) {
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="mb-3">
                  ...
                  <input
                    ...
                    onChange={handleInputChange}
                    placeholder="e.g., Karachi to Hyderabad"
                    required
                  />
                </div>
          ...
          <div className="card">
            <div className="card-header d-flex justify-content-between align-items-center">
              <h5 className="card-title mb-0">
                ...
                Recent Fare Receipts
              </h5>
              <button 
                className="btn btn-outline-primary btn-sm"
                onClick={loadFareReceipts}
            ...
            <div className="card-body">
              {loadingEntries ? (
                <div className="text-center py-4">
                  ...
                  <p className="mt-2">Loading fare receipts...</p>
                </div>
              ) : entries.length === 0 ? (
                <div className="text-center py-4">
                  <i className="bi bi-inbox display-4 text-muted"></i>
                  <p className="text-muted mt-2">No fare receipts found</p>
                </div>
              ) : (
